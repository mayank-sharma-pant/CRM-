"""Mailbox OAuth connect / disconnect / sync."""
...
from app.services.sales.mailbox import (
    delete_user_mailbox,
    exchange_mailbox_code,
    get_user_mailbox,
    mailbox_authorization_url,
    parse_mailbox_state,
    serialize_mailbox,
    sync_mailbox,
    upsert_connection,
...
def _error_redirect(code: str) -> RedirectResponse:
    return RedirectResponse(url=_frontend(f"/settings/email?mailbox_error={code}"), status_code=302)
...
def mailbox_status(
    ...
    row = apply_company_scope(
        db.query(MailboxConnection), MailboxConnection, current_user
    ).filter(MailboxConnection.user_id == current_user.id).first()
    return serialize_mailbox(row)
...
def mailbox_disconnect(
    ...
    if current_user.company_id is None:
        raise HTTPException(status_code=403, detail="User must belong to a company")
    delete_user_mailbox(db, current_user.id)
    return None
...
def mailbox_sync(
    ...
    if current_user.company_id is None:
        raise HTTPException(status_code=403, detail="User must belong to a company")
    row = get_user_mailbox(db, current_user.id)
    if row is None or row.company_id != current_user.company_id:
        raise HTTPException(status_code=400, detail="No mailbox connected")
    try:
        imported = sync_mailbox(db, row)
    except Exception:
        raise HTTPException(status_code=502, detail="Mailbox sync failed")
    return {"imported": imported}


@router.get("/oauth/{provider}/start")
def mailbox_oauth_start(
    provider: str,
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        url = mailbox_authorization_url(provider, current_user.id)
    except OAuthError:
        raise HTTPException(status_code=503, detail="OAuth provider not configured")
...
def mailbox_oauth_callback(
    ...
    try:
        user_id = parse_mailbox_state(state, provider)
        tokens = exchange_mailbox_code(provider, code)
    ...
    return RedirectResponse(url=_frontend("/settings/email?mailbox=success"), status_code=302)
