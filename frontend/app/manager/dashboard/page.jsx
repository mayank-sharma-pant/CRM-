...
/**
 * MANAGER DASHBOARD - Manager Execution View
 * 
 * Purpose: High-level team overview and critical actions.
 * Pattern: Backend-driven data rendering.
 * 
 * Sections:
 * 1. Metrics (Total Team Leads, Team Conversions, Win Rate)
 * 2. Priority Tasks (Escalations/Overdue)
 */
...
export default function ManagerDashboard() {
    ...
    const fetchDashboardData = async () => {
        ...
        if (useMockData) {
            // Generate realistic randoms for demo - slightly higher for manager (team view)
            const total = Math.floor(Math.random() * (450 - 320 + 1)) + 320;
            const closed = Math.floor(Math.random() * (120 - 80 + 1)) + 80;
            const rate = Math.floor(Math.random() * (40 - 20 + 1)) + 20;

            setMetrics({ totalLeads: total, closedLeads: closed, conversionRate: rate });

            setPriorityTasks([
                { id: 201, title: 'Approve Q3 Sales Strategy', dueDate: new Date().toISOString(), statusReason: 'DUE_TODAY' },
                { id: 202, title: 'Team Performance Review - John Doe', dueDate: new Date(Date.now() - 86400000).toISOString(), statusReason: 'OVERDUE' },
                { id: 203, title: 'Client Escalation: TechCorp', dueDate: new Date().toISOString(), statusReason: 'DUE_TODAY' },
                { id: 204, title: 'Review Monthly Revenue Report', dueDate: new Date().toISOString(), statusReason: 'DUE_TODAY' },
                { id: 205, title: 'Approve Commission Payouts', dueDate: new Date(Date.now() + 86400000).toISOString(), statusReason: 'UPCOMING' }
            ]);
        ...
        // For manager, this might aggregate all leads? 
        // For now, using same logic as Sales but ideally would query /team/leads
        ...
        <div className="min-h-[calc(100vh-56px)] bg-slate-50 dark:bg-slate-900 pb-12">
                ...
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                            Team performance overview and priority items
                        </p>
                    </div>
            ...
            <div className="max-w-5xl mx-auto px-8 space-y-8">
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center gap-3 bg-indigo-50/30 dark:bg-indigo-900/10">
                        <ShieldAlert className="text-indigo-600 dark:text-indigo-400" size={20} />
                        <h2 className="text-lg font-bold text-slate-800 dark:text-white">Manager Priorities</h2>
                    </div>
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-700/50">
                        {priorityTasks.length === 0 ? (
                            <div className="p-12 text-center">
                                ...
                                <p className="text-slate-500 text-sm mt-1">No urgent manager tasks.</p>
                            </div>
                        ) : (
                      ...
                      `}>
                                                {task.statusReason === 'OVERDUE' ? 'Attention Needed' : 'Action Required'}
                                            </span>
                                        </div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700 px-6 py-3">
                        <Link href="/manager/tasks" className="text-sm font-medium text-slate-600 hover:text-blue-600 dark:text-slate-400 dark:hover:text-blue-400 flex items-center gap-1 transition-colors">
                            View all manager tasks <ArrowRight size={14} />
                        </Link>
                    </div>
                </div>
