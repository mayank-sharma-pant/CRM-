...
import { Plus, ChevronRight, Filter, Briefcase, X } from 'lucide-react';
...
export default function ManagerLeads() {
    ...
    const safeParseISO = (s) => {
        ...
        try { const d = parseISO(s); return isNaN(d.getTime()) ? null : d; } catch { return null; }
    };
            ...
            <div className="bg-surface border-b border-border px-6 py-4">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    ...
                    <button onClick={() => setShowCreate(true)}
                        ...
                        <Plus size={14} strokeWidth={3} /> Create Lead
                    </button>
                </div>
            </div>
                ...
                <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
                    <div className="bg-surface rounded-xl shadow-2xl border border-border w-full max-w-md" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                            <h3 className="text-base font-bold text-primary">Create New Lead</h3>
                    ...
                    {TABS.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={`px-3 py-1 text-[11px] font-bold uppercase tracking-wider rounded-md transition-all ${activeTab === tab.id
                                ? 'bg-surface-elevated text-accent ring-1 ring-border' : 'text-muted hover:text-secondary hover:bg-surface-elevated'}`}>
                            {tab.label}
                        </button>
                    ))}
                </div>
                            ...
                            return (
                                <Link key={lead.id} href={`/manager/leads/${lead.id}`}
                                    ...
                                    <div className="w-[35%] min-w-[200px]">
                                        <p className={`text-[13px] font-bold truncate tracking-tight ${isMuted ? 'text-muted' : 'text-primary group-hover:text-accent transition-colors'}`}>{lead.name}</p>
                                    ...
                                    <div className="flex-1 flex items-center gap-8 min-w-0">
                                        ...
                                        {signal.text && <span className={`text-[13px] font-medium truncate ${isMuted ? 'text-muted' : signal.color}`}>{signal.text}</span>}
                                    </div>
