...
export default function MomentumChart({ data }) {
    ...
    if (!mounted) return <div style={{ width: '100%', height: '220px' }} className="bg-surface-elevated/5 animate-pulse rounded" />;
    if (!data || !Array.isArray(data) || data.length === 0) return <div style={{ width: '100%', height: '220px' }} className="flex items-center justify-center text-[10px] text-muted uppercase font-bold bg-surface-elevated/5 rounded">No Data Available</div>;
